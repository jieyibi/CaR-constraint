...
def merge_runs(args: Namespace) -> None:
    ...
    sorted_tuples: list[tuple[ApiRun, dict[str, Union[int, float]]]] = sorted(
    ...
    response = input(f"Continue with merge? (y/N): ")
    if response.lower() == "y":
        print(f"Merging runs (response=y)")
    else:
        print(f"Skipping merge (response=n)")
        return
    ...
    if args.merge_run_save_dir is not None:
        ...
        raise RuntimeError(
            f"Missing both --merge-run-save-dir and 'work_dir' in config."
            f" Must use --merge-run-save-dir if 'work_dir' doesn't exist in config."
        )
    ...
    if save_dir is None:
        raise RuntimeError(
            f"Failed to find save dir for creating run. Run config is missing"
            f" 'wandb_init_kwargs' and 'work_dir'. Must specify --into-save-dir"
            f" and re-run."
        )
    ...
    for idx, partial_run in enumerate(matching_runs):
        ...
        # Log history
        num_rows = len([_ for _ in history])
        for logged_dict in tqdm(history, total=num_rows, desc=f"{header}: History"):

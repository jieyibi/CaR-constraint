...
def print_solution(data, manager, routing, assignment, problem="CVRP", log_file=None):
    ...
    for vehicle_id in xrange(data['num_vehicles']):
        ...
        while not routing.IsEnd(index):
            ...
            if problem == "TSPDL" and 'draft' in data:
...
if __name__ == '__main__':
    ...
    for dataset_path in opts.datasets:
        ...
        def run_func(args):
            ...
            depot, loc, demand, capacity, route_limit, service_time, tw_start, tw_end, draft, all_loc = None, None, None, None, None, None, None, None, None, None
        ...
        if opts.lkh is not None:
            with open(opts.lkh, 'rb') as file:
                lkh = pickle.load(file)
            gaps = []
            for i in range(opts.n):
                if results[i][0] is not None and lkh[i][0] is not None:
                    gap = (results[i][0] - lkh[i][0]) / lkh[i][0]
                    gaps.append(gap)
            if len(gaps) > 0:
                print("GAP: {:.2f}%".format(np.mean(gaps)*100))

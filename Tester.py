...
class Tester:
    def __init__(self, args, env_params, model_params, tester_params):
        ...
        if self.tester_params["lazy_piggy_model"] and self.tester_params["use_sl_mask"]:
            self.lazy_model = get_model(self.args.model_type)(**self.model_params)
        else:
            self.lazy_model = None

        if args.lazy_checkpoint and self.tester_params["use_sl_mask"]:
            checkpoint_fullname = args.lazy_checkpoint
            ...
            try:
                print(">> Load SL lazy model from {} [Accuracy: {:.4f}%; Infeasible: {:.4f}%; Feasible: {:.4f}%]".format(
                        checkpoint_fullname, checkpoint['accuracy'] * 100, checkpoint['infsb_accuracy'] * 100,
                                             checkpoint['fsb_accuracy'] * 100))
            except:
                print(">> Load SL lazy model from {}".format(checkpoint_fullname))

        if self.model_params["load_safety_layer"]:
            from models.SINGLEModel import Safety_Layer
            self.safety_layer = Safety_Layer(**self.model_params)
            num_param(self.safety_layer)
            sl_checkpoint = torch.load(args.sl_checkpoint, map_location=self.device)
            try:
                self.safety_layer.load_state_dict(sl_checkpoint["model_state_dict"], strict=True)
            except:
                self.safety_layer.load_state_dict(sl_checkpoint, strict=True)
            print(">> Safety Layer Checkpoint ({}) Loaded!".format(args.sl_checkpoint))
        else:
    ...
    def run(self):
        for env_class in self.envs:
            ...
            if self.tester_params['test_set_path'] is None or self.tester_params['test_set_path'].endswith(".pkl"):
                ...
                excel_file = "tsptw_lib_5075.xlsx"
    ...
    def _test(self, env_class, compute_gap=False):
        ...
        while episode < test_num_episode:
            ...
            if compute_gap:
                ...
                # opt_sol = [i[0] /100 for i in opt_sol]
            ...
            if all_done:
                ...
                import pandas as pd
                # 转换为Pandas DataFrame
                print(opt_sols.unsqueeze(-1).size(), scores.size(), aug_scores.unsqueeze(-1).size(),
                      no_aug_feasibles.unsqueeze(-1).size(), aug_feasibles.unsqueeze(-1).size())
                tensor_data = torch.cat([opt_sols.unsqueeze(-1) ,scores, aug_scores.unsqueeze(-1),
                                         no_aug_feasibles.unsqueeze(-1), aug_feasibles.unsqueeze(-1)], dim =-1)
                # tensor_data = torch.cat([scores, aug_scores.unsqueeze(-1),
                #                          no_aug_feasibles.unsqueeze(-1), aug_feasibles.unsqueeze(-1)], dim =-1)
    ...
    def _test_one_batch(self, test_data, env):
        ...
        with torch.inference_mode():
            ...
            if self.model_params["double_head_decoder"] and self.lazy_model is not None:
        ...
        while not done:
            if self.args.problem == "TSPTW":
                candidate_feature = env.node_tw_end if self.model_params["add_candidate_feature"] else None
            elif self.args.problem == "TSPDL":
                candidate_feature = env.node_draft_limit if self.model_params["add_candidate_feature"] else None
            else:
                candidate_feature = None
            feasible_start = env.simulated_ninf_flag if self.env_params["pomo_feasible_start"] and env.selected_count == 1 else None

            use_sl_mask = True if (self.model_params['double_head_decoder'] and self.tester_params['use_sl_mask']) else False
            # print(use_sl_mask)
            ...
            #                                       no_sigmoid=True)
            selected, prob, probs_return = self.model(state, pomo=self.env_params["pomo_start"],feasible_start = feasible_start,
                                                      candidate_feature=candidate_feature, use_sl_mask=use_sl_mask, no_sigmoid=True)
            # if self.model_params["load_safety_layer"]:
            ...
            #     elif self.args.problem == "TSPDL":
            #         candidate_feature = env.node_draft_limit if self.model_params["add_candidate_feature"] else None
            #     selected, _, _ = self.model(state, safety_layer = sl, route = env.selected_node_list,
            #                                 pomo=self.env_params["pomo_start"], candidate_feature=candidate_feature)
            # else:
            #     selected, _ = self.model(state, pomo=self.env_params["pomo_start"])
            # shape: (batch, pomo)
            simulation = self.model_params["simulation_fsb"]
            if self.env_params["pomo_feasible_start"] and env.selected_count == 0:
                simulation = True
            state, reward, done, infeasible = env.step(selected, simulation=simulation)
